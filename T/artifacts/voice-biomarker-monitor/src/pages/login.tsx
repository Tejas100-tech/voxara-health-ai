...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col lg:flex-row overflow-hidden">
      {/* Left panel */}
      <div className="relative flex-1 bg-gradient-to-br from-[#011C40] via-[#023859] to-[#011C40] flex flex-col justify-between p-10 min-h-[260px] lg:min-h-screen overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-[520px] h-[520px] bg-cyan-500/15 rounded-full blur-[100px] translate-x-1/2 -translate-y-1/2" />
          <div className="absolute bottom-0 left-0 w-[380px] h-[380px] bg-sky-500/10 rounded-full blur-[80px] -translate-x-1/3 translate-y-1/3" />
        </div>
        ...
        <div className="relative z-10 max-w-xl">
          <h2 className="text-4xl md:text-5xl font-extrabold text-white font-[Manrope] leading-tight mb-6">
            {t("landing.heroTitle")} <span className="text-cyan-400">{t("landing.heroHighlight")}</span>
          </h2>
        ...
        <div className="w-full max-w-md">
          <div className="mb-10">
            ...
            <p className="text-muted-foreground">
              {t("login.description")}{" "}
              <Link href="/signup" className="text-cyan-600 dark:text-cyan-400 font-bold hover:underline">{t("login.createAccount")}</Link>
            </p>
          </div>
            ...
            {DEMO_ACCOUNTS.map((account) => {
              ...
              return (
                <button key={account.email} onClick={() => handleDemoLogin(account.email, account.password)} disabled={loading}
                  ...
                  <div className="w-10 h-10 rounded-xl bg-cyan-100 dark:bg-cyan-900/40 text-cyan-600 dark:text-cyan-400 flex items-center justify-center shrink-0 group-hover:bg-cyan-500 group-hover:text-white transition-all">
                    <Icon size={20} />
                  </div>
